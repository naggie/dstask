...
GOOS=linux GOARCH=arm GOARM=5 go build -o dstask-sync -mod=vendor -ldflags="$LDFLAGS" cmd/dstask-sync/main.go
# upx -q dstask
mv dstask dist/dstask-linux-arm5
mv dstask-sync dist/dstask-linux-arm5

GOOS=linux GOARCH=amd64 go build -o dstask -mod=vendor -ldflags="$LDFLAGS" cmd/dstask/main.go
GOOS=linux GOARCH=amd64 go build -o dstask-sync -mod=vendor -ldflags="$LDFLAGS" cmd/dstask-sync/main.go
# upx -q dstask
mv dstask dist/dstask-linux-amd64
mv dstask-sync dist/dstask-linux-amd64

GOOS=darwin GOARCH=amd64 go build -o dstask -mod=vendor -ldflags="$LDFLAGS" cmd/dstask/main.go
GOOS=darwin GOARCH=amd64 go build -o dstask-sync -mod=vendor -ldflags="$LDFLAGS" cmd/dstask-sync/main.go
#upx -q dstask
mv dstask dist/dstask-darwin-amd64
mv dstask-sync dist/dstask-darwin-amd64
